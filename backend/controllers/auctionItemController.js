...
export const addNewAuctionItem = catchAsyncErrors(async(req,res,next)=>{
    ...
    if(!allowedFormats.includes(image.mimetype)){
        return next(new ErrorHandler("Only .png, .jpg and .webp format allowed!", 400));
    }
    ...
    if(new Date(startTime) < Date.now()){
        return next(new ErrorHandler("Auction starting time must be greater that present time.",400))
    }

    if(new Date(startTime) >= new Date(endTime)){
        return next(new ErrorHandler("Auction starting time must be less that ending time.",400))
    }
...
export const republishItem = catchAsyncErrors(async(req, res, next)=>{
    ...
    if(data.startTime < Date.now()){
        return next(new ErrorHandler("Auction starting time must be greater than presen time", 400));
    };
